...
# Beat the C011 deduped null baseline by this margin (per-R).
...
def evaluate_validation_gates(
    ...
    *,
    ...
    checks = {
        ...
        "validation_pf_gte_1_05": bool((val.get("profit_factor") or 0) >= MIN_VALIDATION_PF),
        "validation_trade_count_gte_100": bool(val.get("trade_count", 0) >= MIN_VALIDATION_TRADES),
    ...
    screening_pass = train_pass and all(
        ...
        if k not in ("backtrader_parity_pass", "train_expectancy_gte_zero")
    )
    verdict = "SCREENING_PASS" if (train_pass and screening_pass) else "REJECT"
    return {
        ...
        # SINGLE_PAIR_REVIEW_ONLY is a precommitted *non-PASS* fallback: if the
        # 4/7-pair gate fails but one pair is materially strong, a later sprint
        # classifies SINGLE_PAIR_REVIEW_ONLY — never an automatic PASS.
        "single_pair_review_only_allowed": bool(
            train_pass and not checks["validation_pairs_positive_gte_4_of_7"]
        ),
    }
