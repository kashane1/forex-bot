...
> **Parameters: use the generated config, not the table below.** The
> parameter table in this file was hand-copied from the *frozen
> baseline* (`configs/paper.yaml`) and is **not** CAMPAIGN_002's actual
> config — CAMPAIGN_002 used `atr_stop_multiple` 2.0 (not 2.5),
> `max_bars_in_trade` 240 (not 80), and an empty `min_atr_pips`. The
> authoritative parameters are `research/lean_parity/lean_parity_config.json`,
> generated by `scripts/build_lean_parity_config.py` directly from the
> committed campaign config. See `docs/research/LEAN_PARITY_EXECUTION_GUIDE.md`.
