...
Reads the per-fold trade CSVs produced by ``run_campaign_010.py``,
...
Emits:
  ...
  * backtests/CAMPAIGN_010_session_breakout/financing/financing_summary.json
    (extended per-fold / per-pair / long-short breakdown)

Strict rules:
  * No broker call, no credential read.
  * MODELED treatment is refused at the source layer (the call would
    raise); this script uses ESTIMATED only.
  * Engine PnL is unchanged. The overlay is additive context, not a
    PnL rewrite.
...
def _load_trades(folds_dir: Path) -> list[tuple[PositionInterval, dict[str, str]]]:
    """Walk folds/fold_NN/fold_NN_<PAIR>_trades.csv and yield
    (PositionInterval, raw-row-dict)."""
...
def _aggregate(
    ...
    summaries: list,  # PositionFinancingSummary
) -> dict:
...
def main() -> int:
    ...
    extra_payload = {
        ...
        "trade_pnl": {
            "total_usd": extra["total_trade_pnl_usd"],
        },
    ...
    (fin_dir / "financing_summary.json").write_text(
        json.dumps(extra_payload, indent=2, default=str),
        encoding="utf-8",
    )
