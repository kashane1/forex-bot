#!/usr/bin/env python3
"""CAMPAIGN_016 walk-forward evidence runner — weekly cross-sectional momentum 0.1.0-c016.

Drives the weekly cross-sectional momentum low-turnover candidate against
each fold's test window from a rolling walk-forward plan, on the 7-pair H4
OANDA practice universe, and emits per-fold + aggregate JSON plus a
machine-readable ``gate_result.json`` consumed by the Phase 3 result doc and
Phase 4 anti-overfit diagnostics.

**CAMPAIGN_016 is a research candidate scaffold; this runner produces
...
Strict rules — enforced by this script:
  ...
  * The strategy must be ``weekly_cross_sectional_momentum_low_turnover``
    (i.e. ``strategy.enabled == ['weekly_cross_sectional_momentum_low_turnover']``).
  * Strategy parameters must match the pre-commit verbatim
    (``CAMPAIGN_016_WEEKLY_CROSS_SECTIONAL_MOMENTUM_PRECOMMIT.md`` §5).
  ...
  * Every per-pair per-fold candle source must be ``oanda-practice``.
    ...
    as ``BLOCKED`` (Phase 0 §10 BLOCKED-conditions).
  ...
  * The bespoke engine is the canonical evidence path; Backtrader is
    a secondary verification lane (Phase 9).
  * No tuning. No parameter sweep. No retry with altered parameters
    to improve results.

**CROSS-PAIR RUNNER INTEGRATION CONTRACT (binding):**

  The strategy requires sibling-pair H4 close series via
  ``ctx.config["cross_pair_h4_closes"]``. The runner MUST:

    1. Load all 7 pairs' completed H4 candles for the test window
       + warm-up margin (400 calendar days).
...
EXPECTED_STRATEGY = "weekly_cross_sectional_momentum_low_turnover"
EXPECTED_PAIRS = (
    "EUR_USD",
    "GBP_USD",
    "USD_JPY",
...
# CAMPAIGN_016_WEEKLY_CROSS_SECTIONAL_MOMENTUM_PRECOMMIT.md §5). Any mismatch
...
# Walk-forward windows (pre-committed, frozen by Phase 0 §6).
PLAN_WINDOWS = {
...
# Universe windows match CAMPAIGN_010 / 011 / 012 / 013 / 014 / 015 verbatim
# so the 2025-2026 lockbox discipline is preserved.
UNIVERSE_START = date(2020, 1, 1)
UNIVERSE_END = date(2026, 5, 20)

# Minimum aligned H4 bars required by the strategy warmup contract.
MIN_COMMON_INDEX_BARS = 420

# Aggregate gates (Phase 0 §7). The bespoke runner emits a verdict
...
class CrossPairFoldContext:
    """Per-fold cross-pair H4 closes context.

    Built once per fold: holds the 7-pair aligned H4 closes (one
    ``pd.Series`` per pair) and runner-level diagnostics.
    """

    fold_index: int
    cross_pair_h4_closes: dict[str, pd.Series]
...
def _fold_dates_to_dts(fold: Fold) -> tuple[datetime, datetime]:
    ...
    inclusive day range — PLUS warm-up margin (400 calendar days) so
    12-week weekly lookback + H4 ATR(14) burn-in are fully warm at fold
...
def _build_cross_pair_context(
    *,
    ...
    """Build the cross-pair H4 closes dict for this fold (binding contract).

    Loads all 7 pairs' completed H4 candles in the (test window +
    warm-up) window via ``list_with_dedupe_stats``; verifies each
    pair's data source is ``oanda-practice``; aligns all 7 pairs to a
    common timestamp index (intersection of completed bars); builds
    per-pair closes-only ``pd.Series``; returns a ``CrossPairFoldContext``
    with the aligned dict + diagnostics.
    ...
    for pair in EXPECTED_PAIRS:
        ...
        if not frame.df.index.is_unique:
            return CrossPairFoldContext(
                ...
                contract_satisfied=False,
                blocker=(
                    f"pair {pair} fold {fold.fold_index} has non-unique "
                    f"CandleFrame index after dedupe "
                    f"(duplicates_detected={dedupe_stats.duplicates_detected})"
                ),
            )
    ...
    if len(common_index) < MIN_COMMON_INDEX_BARS:
        return CrossPairFoldContext(
            fold_index=fold.fold_index,
            cross_pair_h4_closes={},
            ...
            blocker=(
                ...
                f"need >= {MIN_COMMON_INDEX_BARS} for strategy warmup"
            ),
        )
    ...
    *,
    ...
    if src != REQUIRED_DATA_SOURCE:
        raise SystemExit(
            ...
            f"{REQUIRED_DATA_SOURCE!r}. CAMPAIGN_016 aborts."
        )
    frm, to = _fold_dates_to_dts(fold)
    rows, dedupe_stats = candle_repo.list_with_dedupe_stats(
    ...
    strategy_cfg_with_cross_pair["cross_pair_h4_closes"] = (
        cross_pair_ctx.cross_pair_h4_closes
    )
    ...
    engine = BacktestEngine(
        instrument=meta,
        strategy=WeeklyCrossSectionalMomentumLowTurnoverStrategy(
...
class FoldRollup:
    ...
    cross_pair_diagnostics: dict[str, Any] = field(default_factory=dict)
    ...
    def gate_vector(self) -> dict[str, bool]:
        return {
            "trade_count_ge_15": self.trade_count >= PER_FOLD_GATES["trade_count_min"],
            "expectancy_r_ge_0": self.expectancy_r >= PER_FOLD_GATES["expectancy_r_min"],
            "pairs_positive_ge_2": self.pairs_positive >= PER_FOLD_GATES["pairs_positive_min"],
...
def _aggregate(rollups: list[FoldRollup], *, cost_label: str) -> dict[str, Any]:
    ...
    aggregate_gate_vector = {
        ...
        "trade_count_min_120": total_trades >= AGGREGATE_GATES["trade_count_min"],
        "trade_count_max_500": total_trades <= AGGREGATE_GATES["trade_count_max"],
        "pairs_positive_ge_4_of_7": pairs_positive_count
        >= AGGREGATE_GATES["pairs_positive_min"],
        "single_pair_dominance_le_60pct": single_pair_dom_pct
...
def _assert_frozen(strategy_cfg: dict[str, Any]) -> None:
    ...
    if mismatched:
        raise SystemExit(
            "CAMPAIGN_016 frozen-parameter mismatch — see "
            "CAMPAIGN_016_WEEKLY_CROSS_SECTIONAL_MOMENTUM_PRECOMMIT.md §5:\n"
            + "\n".join(mismatched)
        )
...
# ---------------------------------------------------------------------------
# Local data preflight (BLOCKED conditions; Phase 0 §10)
...
def _data_preflight(
    *,
    ...
    cross_pair_blockers: list[str] = []
    for fold in plan.folds:
        ctx = _build_cross_pair_context(
            fold=fold,
            candle_repo=candle_repo,
            ds_repo=ds_repo,
        )
        fold_key = f"fold_{fold.fold_index:02d}"
        details["cross_pair_contract"][fold_key] = {
            "contract_satisfied": ctx.contract_satisfied,
            "common_index_length": ctx.common_index_length,
            "per_pair_raw_lengths": ctx.per_pair_raw_lengths,
            "blocker": ctx.blocker,
        }
        if not ctx.contract_satisfied:
            cross_pair_blockers.append(
                f"fold {fold.fold_index} cross-pair contract failed: {ctx.blocker}"
            )
    reasons.extend(cross_pair_blockers)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        sc.enabled != [EXPECTED_STRATEGY]
        or sc.weekly_cross_sectional_momentum_low_turnover is None
    ):
        raise SystemExit(
            f"CAMPAIGN_016 config must enable only {EXPECTED_STRATEGY!r}; "
            f"got {sc.enabled}"
        )
    strategy_cfg = sc.weekly_cross_sectional_momentum_low_turnover.model_dump()
    _assert_frozen(strategy_cfg)
    if strategy_cfg["entry_timing"] != "next_bar_open":
        raise SystemExit(
            f"CAMPAIGN_016 entry_timing must be next_bar_open; "
            f"got {strategy_cfg['entry_timing']!r}"
        )

    pairs = tuple(settings.market.instruments)
    if pairs != EXPECTED_PAIRS:
        raise SystemExit(
            f"CAMPAIGN_016 universe mismatch — got {pairs}, expected "
            f"{EXPECTED_PAIRS}"
        )

    plan = rolling_window_plan(
        campaign_name="CAMPAIGN_016_weekly_cross_sectional_momentum",
        ...
        notes=[
            "CAMPAIGN_016 walk-forward plan (Phase 0 §6); 8 folds rolling, "
    ...
    print(
        f"=== CAMPAIGN_016 walk-forward run started "
    ...
    print(
        f"frozen params: fast={strategy_cfg['momentum_lookback_fast_weeks']}w "
        f"slow={strategy_cfg['momentum_lookback_slow_weeks']}w "
        f"blend={strategy_cfg['momentum_blend_fast']}/{strategy_cfg['momentum_blend_slow']} "
    ...
    base_rollups = rollups_by_cost.get("base", [])
    ...
    base_agg = cost_runs.get("base", {})
    stress_agg = cost_runs.get("2xcost", {})

    aggregate_model = AggregateMetrics(
        fold_count=base_agg.get("fold_count", 0),
        folds_passing_gates=base_agg.get("folds_passing", 0),
        fold_pass_rate=base_agg.get("fold_pass_rate", 0.0),
        total_trades_across_folds=base_agg.get("total_trades", 0),
        aggregate_expectancy_r=base_agg.get("aggregate_expectancy_r", 0.0),
        aggregate_return_pct=base_agg.get("aggregate_return_pct", 0.0),
    ...
    fold_detail: dict[str, Any] = {
        ...
        "approval_path": (
            ...
            "CAMPAIGN_016_WEEKLY_CROSS_SECTIONAL_MOMENTUM_PRECOMMIT.md §11 "
        ...
        "cross_pair_runner_contract": {
            "min_common_index_bars": MIN_COMMON_INDEX_BARS,
    ...
    gate_result = {
        ...
        "config_path": str(config_path),
        "config_hash": settings.config_hash,
        "fold_count": base_agg.get("fold_count", len(plan.folds)),
        ...
        "by_cost": {
            "base": {
                "aggregate_expectancy_r": base_agg.get("aggregate_expectancy_r"),
                "aggregate_return_pct": base_agg.get("aggregate_return_pct"),
                "profit_factor": base_agg.get("profit_factor"),
                "total_trades": base_agg.get("total_trades"),
                "fold_pass_rate": base_agg.get("fold_pass_rate"),
                "folds_passing": base_agg.get("folds_passing"),
                "pairs_positive_count": base_agg.get("pairs_positive_count"),
                "single_pair_dominance_pct": base_agg.get("single_pair_dominance_pct"),
                "median_per_fold_expectancy_r": base_agg.get("median_per_fold_expectancy_r"),
                "trade_level_cumulative_r": base_agg.get("trade_level_cumulative_r"),
                "aggregate_gates": base_agg.get("aggregate_gates"),
                "aggregate_pass": base_agg.get("aggregate_pass"),
                "expectancy_min_applied": base_agg.get("expectancy_min_applied"),
                "profit_factor_min_applied": base_agg.get("profit_factor_min_applied"),
            },
            "2xcost": {
                "aggregate_expectancy_r": stress_agg.get("aggregate_expectancy_r"),
                "aggregate_return_pct": stress_agg.get("aggregate_return_pct"),
                "profit_factor": stress_agg.get("profit_factor"),
                "total_trades": stress_agg.get("total_trades"),
                "fold_pass_rate": stress_agg.get("fold_pass_rate"),
                "folds_passing": stress_agg.get("folds_passing"),
                "aggregate_gates": stress_agg.get("aggregate_gates"),
                "aggregate_pass": stress_agg.get("aggregate_pass"),
                "expectancy_min_applied": stress_agg.get("expectancy_min_applied"),
                "profit_factor_min_applied": stress_agg.get("profit_factor_min_applied"),
            },
        },
        "blocked": blocked_verdict,
        "blocked_reasons": [f"fold {fi}: {blk}" for fi, blk in blocked_folds],
        "generated_at": datetime.now(UTC).isoformat(),
    }
    (wf_dir / "gate_result.json").write_text(
        json.dumps(gate_result, indent=2, default=str), encoding="utf-8",
    ...
    print(
        ...
        "DO NOT add weekly_cross_sectional_momentum_low_turnover to the registry."
    )
    return 0
...
def _classify(*, base_agg: dict[str, Any], stress_agg: dict[str, Any]) -> str:
    ...
    if base_agg.get("aggregate_pass") and stress_agg.get("aggregate_pass"):
