#!/usr/bin/env python3
"""CAMPAIGN_019 — mean_reversion_thesis_invalidation execution runner.
...
OUT_BT = ROOT / "backtests/CAMPAIGN_019_mean_reversion_thesis_invalidation"
OUT_RESEARCH = ROOT / "research/campaign_019"
...
def validate_frozen_config(settings) -> tuple[float, float, int]:
    sc = settings.strategy
    if sc.enabled != ["mean_reversion_thesis_invalidation"]:
        raise SystemExit("CAMPAIGN_019 config must enable only mean_reversion_thesis_invalidation")
    if sc.mean_reversion_thesis_invalidation is None:
        raise SystemExit("missing mean_reversion_thesis_invalidation config")
    ...
    if c008_entry_params(c019) != c008_entry_params(c008):
        raise SystemExit("C019 entry params diverge from C008 frozen config")
    ti = c019["thesis_invalidation"]
    if ti["long_exit_zscore"] != -3.0 or ti["short_exit_zscore"] != 3.0:
        raise SystemExit("thesis_invalidation z thresholds not precommitted")
    if ti["zscore_lookback"] != 20:
        raise SystemExit("thesis_invalidation zscore_lookback must be 20")
    return float(ti["long_exit_zscore"]), float(ti["short_exit_zscore"]), int(ti["zscore_lookback"])
...
def load_ctx() -> CampaignCtx:
    ...
    long_z, short_z, z_len = validate_frozen_config(settings)
    ...
    return CampaignCtx(
        ...
        thesis_long_z=long_z,
        thesis_short_z=short_z,
        thesis_zscore_lookback=z_len,
...
def run_pair_split(
    ...
    *,
    ...
    engine = BacktestEngine(
        instrument=meta,
        strategy=MeanReversionThesisInvalidationStrategy(version=ctx.strategy_cfg["version"]),
        ...
        thesis_invalidation_long_z=ctx.thesis_long_z,
        thesis_invalidation_short_z=ctx.thesis_short_z,
        thesis_invalidation_zscore_lookback=ctx.thesis_zscore_lookback,
    )
    result = engine.run(frame, data_request_hash=data_hash)
    regime_name = str(regime["name"])
    sub = OUT_BT / split / regime_name
    label = f"c019_{instrument}_{split}_{regime_name}"
...
def mechanism_diagnostics(trades: pd.DataFrame) -> dict[str, Any]:
    ...
    thesis_exit = (
        (trades["exit_reason"] == "thesis_invalidation").sum()
        if "exit_reason" in trades
        else 0
    )
    prot_exit = (
        (trades["exit_reason"] == "protective_stop").sum()
        if "exit_reason" in trades
        else 0
    )
    ...
    time_trades = trades[trades["exit_reason"] == "time"] if "exit_reason" in trades else pd.DataFrame()
    time_mfe_median = None
    if not time_trades.empty and "r_multiple" in time_trades:
        time_mfe_median = round(float(time_trades["r_multiple"].median()), 4)
    zscores = []
    if "zscore_at_exit" in trades.columns:
        zscores = [
            float(z)
            for z in trades.loc[trades["exit_reason"] == "thesis_invalidation", "zscore_at_exit"]
            if pd.notna(z) and str(z) != ""
        ]
    return {
        "total_trades": len(trades),
        "thesis_invalidation_exit_count": int(thesis_exit),
        "thesis_invalidation_exit_rate_pct": round(100.0 * thesis_exit / len(trades), 2),
        "protective_stop_exit_count": int(prot_exit),
        ...
        "time_exit_median_r": time_mfe_median,
        "thesis_invalidation_zscore_median": (
            round(float(statistics.median(zscores)), 4) if zscores else None
        ),
        "thesis_invalidation_zscore_samples": len(zscores),
    }
...
def load_baseline_comparison() -> dict[str, Any]:
    ...
    if C018_RESEARCH.exists():
        c018_train = json.loads((C018_RESEARCH / "train_metrics.json").read_text())
        c018_val = json.loads((C018_RESEARCH / "validation_metrics.json").read_text())
        out["c018_executed"] = {"train": c018_train, "validation": c018_val}
    return out
...
def write_artifacts(payload: dict[str, Any]) -> None:
    ...
    (OUT_RESEARCH / "comparison_to_c011_null.json").write_text(
        json.dumps(
            {k: comp[k] for k in ("c019_validation", "c011_null", "beat_null_threshold") if k in comp},
            indent=2,
            default=str,
        ),
        encoding="utf-8",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CAMPAIGN_019 runner")
    parser.add_argument(
        "command",
    ...
    if args.command in ("train-validation", "full"):
        ...
        payload["comparison"] = {
            **baseline,
            "c019": agg,
            "c019_validation": agg.get("validation"),
            "beat_null_threshold": C011_NULL_EXP_R + BEAT_NULL_MARGIN,
        }
