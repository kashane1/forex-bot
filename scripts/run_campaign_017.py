#!/usr/bin/env python3
"""CAMPAIGN_017 walk-forward evidence runner — weekly_volatility_contraction_breakout 0.1.0-c017.

Drives the weekly volatility contraction breakout candidate against each
fold's test window from a rolling walk-forward plan, on the 7-pair H4 OANDA
practice universe, and emits per-fold + aggregate JSON plus a machine-readable
``gate_result.json`` consumed by the Phase 4 result doc and Phase 5
anti-overfit diagnostics.

**CAMPAIGN_017 is a research candidate scaffold; this runner produces
...
Strict rules — enforced by this script:
  ...
  * The strategy must be ``weekly_volatility_contraction_breakout``
    (i.e. ``strategy.enabled == ['weekly_volatility_contraction_breakout']``).
  * Strategy parameters must match the pre-commit verbatim
    (``CAMPAIGN_017_WEEKLY_VOLATILITY_CONTRACTION_BREAKOUT_PRECOMMIT.md`` §5).
...
EXPECTED_STRATEGY = "weekly_volatility_contraction_breakout"
EXPECTED_PAIRS = (
    "EUR_USD",
    "GBP_USD",
    "USD_JPY",
...
# CAMPAIGN_017_WEEKLY_VOLATILITY_CONTRACTION_BREAKOUT_PRECOMMIT.md §5). Any mismatch
...
def _fold_dates_to_dts(fold: Fold) -> tuple[datetime, datetime]:
    ...
    H4 ATR(14) / 12-week compression lookback are fully warm at fold
    ...
    *,
    ...
    if src != REQUIRED_DATA_SOURCE:
        raise SystemExit(
            ...
            f"{REQUIRED_DATA_SOURCE!r}. CAMPAIGN_017 aborts."
        )
    ...
    engine = BacktestEngine(
        instrument=meta,
        strategy=WeeklyVolatilityContractionBreakoutStrategy(version=strategy_cfg["version"]),
        ...
        atr_lookback=int(strategy_cfg["atr_lookback_h4"]),
...
class FoldRollup:
    ...
    def gate_vector(self) -> dict[str, bool]:
        return {
            "trade_count_ge_12": self.trade_count >= PER_FOLD_GATES["trade_count_min"],
...
def _aggregate(rollups: list[FoldRollup], *, cost_label: str) -> dict[str, Any]:
    ...
    aggregate_gate_vector = {
        ...
        "trade_count_max_350": total_trades <= AGGREGATE_GATES["trade_count_max"],
...
def _assert_frozen(strategy_cfg: dict[str, Any]) -> None:
    ...
    if mismatched:
        raise SystemExit(
            "CAMPAIGN_017 frozen-parameter mismatch — see "
            "CAMPAIGN_017_WEEKLY_VOLATILITY_CONTRACTION_BREAKOUT_PRECOMMIT.md §5:\n"
            + "\n".join(mismatched)
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if sc.enabled != [EXPECTED_STRATEGY] or sc.weekly_volatility_contraction_breakout is None:
        raise SystemExit(
            f"CAMPAIGN_017 config must enable only {EXPECTED_STRATEGY!r}; "
            f"got {sc.enabled}"
        )
    strategy_cfg = sc.weekly_volatility_contraction_breakout.model_dump()
    _assert_frozen(strategy_cfg)

    pairs = tuple(settings.market.instruments)
    if pairs != EXPECTED_PAIRS:
        raise SystemExit(
            f"CAMPAIGN_017 universe mismatch — got {pairs}, expected "
            f"{EXPECTED_PAIRS}"
        )

    # Build the plan in-process from the frozen windows.
    plan = rolling_window_plan(
        campaign_name="CAMPAIGN_017_weekly_volatility_contraction_breakout",
        ...
        notes=[
            "CAMPAIGN_017 walk-forward plan (Phase 0 §7); 8 folds rolling, "
    ...
    print(
        f"=== CAMPAIGN_017 walk-forward run started "
    ...
    print(
        f"frozen params: compression_weeks={strategy_cfg['compression_lookback_weeks']} "
        f"pct={strategy_cfg['compression_percentile_threshold']} "
        f"buffer={strategy_cfg['breakout_buffer_atr_multiple']}*ATR "
    ...
    fold_detail: dict[str, Any] = {
        ...
        "approval_path": (
            ...
            "CAMPAIGN_017_WEEKLY_VOLATILITY_CONTRACTION_BREAKOUT_PRECOMMIT.md §16 "
    ...
    print(
        ...
        "DO NOT add weekly_volatility_contraction_breakout to the registry."
    )
    return 0
