#!/usr/bin/env python3
"""CAMPAIGN_003 runner — controlled ADX-filtered trend following.

ONE strategy: trend_following 0.2.0-c003 (frozen baseline + H4-only +
6-pair universe + ADX-14>25 gate). No robustness grid, no optimizer.
...
(provenance hashes already recorded). RiskEngine wired in
(mode="backtest"). Every run exports a per-signal risk_rejections CSV.
Records every run. Writes nothing into CAMPAIGN_001/002/diagnostics
artifacts.
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--config", default=str(ROOT / "configs/campaign_003_controlled_adx.yaml")
    )
    parser.add_argument(
        "--out", default=str(ROOT / "backtests/campaign_003_controlled_adx/runs")
    )
    ...
    t0 = time.time()
    print(f"=== CAMPAIGN_003 baseline {datetime.now(UTC).isoformat()} ===")
    run_baseline(ctx)
    print(f"=== CAMPAIGN_003 cost stress {datetime.now(UTC).isoformat()} ===")
    run_cost_stress(ctx)
    dt = time.time() - t0
