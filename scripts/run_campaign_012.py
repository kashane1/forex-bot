#!/usr/bin/env python3
"""CAMPAIGN_012 walk-forward evidence runner.

Drives ``regime_switcher_atr_percentile 0.1.0-c012`` against each
fold's TEST window from a walk-forward plan, on the 7-pair H4 OANDA
practice universe, and emits per-fold metrics + aggregate + a
campaign-level ``WalkForwardResults`` JSON/Markdown via the existing
``research.walk_forward`` harness.

**CAMPAIGN_012 is a research candidate scaffold; this runner produces
...
Strict rules — enforced by this script:
  ...
  * The strategy must be ``regime_switcher_atr_percentile`` (i.e.
    ``strategy.enabled == ['regime_switcher_atr_percentile']``).
  * The strategy parameters must match the pre-commit verbatim
    (``CAMPAIGN_012_PRECOMMIT_CHECKLIST.md`` §4). Mismatch aborts
...
# CAMPAIGN_012_PRECOMMIT_CHECKLIST.md §4 and
# REGIME_SWITCHER_ATR_PERCENTILE_IMPLEMENTATION_SPEC.md §2). Any
...
# Pre-commit gate thresholds — INHERITED VERBATIM from CAMPAIGN_010 /
# CAMPAIGN_011 pre-commits so the entry-signal comparison is
...
def _fold_dates_to_dts(fold: Fold) -> tuple[datetime, datetime]:
    ...
    inclusive day range — PLUS warm-up margin pulled from prior bars.

    The regime switcher needs at least 75 D1AGG candles
    (= daily_atr_lookback + regime_lookback_days + 1 = 14 + 60 + 1)
    before it can produce a signal. Each D1AGG day requires 6 H4
    candles, so we need at least ~75 * 6 / (5/7) ≈ 630 H4 candles of
    prior history. We pull a generous margin (warm_up_days_margin
    days before the test window) so the strategy's R1 (warm-up) and
    R3 (regime) gates can pass at the very start of the test window.

    The warm-up bars are consumed by the strategy's internal warm-up
    check; only signals at or after test_start contribute to fold
    metrics — the BacktestEngine handles this via its own bar-loop
    bookkeeping (consistent with CAMPAIGN_010 / 011 runners).
    """
    # Warm-up margin: 200 trading days * 6 H4 bars + slack ~= 250 days
    # of calendar buffer (covers weekends). This matches the strategy's
    # 500-bar warm-up requirement comfortably.
    ...
    *,
    ...
    if src != REQUIRED_DATA_SOURCE:
        raise SystemExit(
            ...
            f"{REQUIRED_DATA_SOURCE!r}. CAMPAIGN_012 aborts."
        )
    ...
    engine = BacktestEngine(
        instrument=meta,
        strategy=RegimeSwitcherAtrPercentileStrategy(version=strategy_cfg["version"]),
...
def _assert_frozen(strategy_cfg: dict[str, Any]) -> None:
    ...
    if mismatched:
        raise SystemExit(
            "CAMPAIGN_012 frozen-parameter mismatch — see "
            "CAMPAIGN_012_PRECOMMIT_CHECKLIST.md §4:\n"
            + "\n".join(mismatched)
        )
...
def main() -> int:
    ...
    if sc.enabled != [EXPECTED_STRATEGY] or sc.regime_switcher_atr_percentile is None:
        raise SystemExit(
            f"CAMPAIGN_012 config must enable only {EXPECTED_STRATEGY!r}; "
            f"got {sc.enabled}"
        )
    strategy_cfg = sc.regime_switcher_atr_percentile.model_dump()
    _assert_frozen(strategy_cfg)

    pairs = tuple(settings.market.instruments)
    if pairs != EXPECTED_PAIRS:
        raise SystemExit(
            f"CAMPAIGN_012 universe mismatch — got {pairs}, expected "
            f"{EXPECTED_PAIRS}"
        )
    ...
    if plan.parameter_mode is not ParameterMode.FROZEN:
        raise SystemExit(
            f"CAMPAIGN_012 plan must use parameter_mode=frozen; "
            f"got {plan.parameter_mode.value!r}"
        )
    if plan.split_style is not SplitStyle.ROLLING:
        raise SystemExit(
            f"CAMPAIGN_012 plan must use split_style=rolling; "
    ...
    print(
        f"=== CAMPAIGN_012 walk-forward run started "
        f"{datetime.now(UTC).isoformat()} ==="
    )
    print(f"folds: {len(plan.folds)}  pairs: {len(pairs)}")
    print(
        f"regime gate: P{int(strategy_cfg['regime_percentile_threshold']*100)} "
        f"of trailing-{strategy_cfg['regime_lookback_days']} D1AGG ATR-"
        f"{strategy_cfg['daily_atr_lookback']}; trend filter: "
        f"close[t] vs close[t-{strategy_cfg['trend_lookback_h4_bars']}] "
        f">= {strategy_cfg['min_close_move_atr_fraction']} x prior_atr_h4"
    )
    ...
    print(f"\nINHERITED-GATE VERDICT: {agg['overall_verdict']}")
    if agg["overall_verdict"] == "PASS":
        print(
            ...
            "DO NOT add regime_switcher_atr_percentile to "
