...
- **Next options.** (a) **DONE** — option (a) was executed as the CAMPAIGN_027 scaffold (see §12). (b) Optionally extend the edge-discovery lab with a Postgres M1/M15 data path to screen sub-H1 ideas (currently out-of-scope: lab reads SQLite only, though M1-materialized data exists in research Postgres). (c) Source a new external thesis for the still-null families.
...
## 12. CAMPAIGN_027 filtered H4 z-score reversion — SCAFFOLD_ONLY / PRECOMMITTED (2026-05-28)

- **Sprint.** `research-campaign-027-h4-filtered-zscore-reversion-scaffold-001` (scaffold/precommit, **not** train/validation, **not** approval).
- **What.** Created the CAMPAIGN_027 scaffold (`h4_filtered_zscore_reversion 0.1.0-c027`) for the one front-gate-eligible idea: short-only filtered H4 z-score reversion (20-bar z shifted-1/ddof=1, enter short |z|>=2.5, low-vol ATR-14 simple-mean trailing-250 percentile <=0.33, quiet-session asia/london, `next_bar_open` fill, exit 12-bar time stop + 3xATR protective stop, conservative financing-inclusive cost binding, 7 majors). Froze hypothesis/rule/artifact-contract/kill-conditions/parity-design/future-prompt.
- **Status.** `SCAFFOLD_ONLY / PRECOMMITTED / NOT_RUN / NOT_APPROVED`. No train/validation/test evidence; test lockbox closed; `approved_strategies.yaml` stays `approved: []`; paper/demo/live blocked; C011 null; C025/C026 REJECT.
- **Nine precommitted kill conditions (may not be weakened).** Train/validation expectancy <=0 post-conservative-cost; not pair-robust / single-pair dominated; negative/collapsing recent years (esp. 2024/2026); matched-null no longer beaten; filters only reduce sample; 2× cost stress fails; Backtrader parity unattainable; required artifact ledgers missing/incompatible.
- **Next (not authorized here).** Run [`NEXT_SPRINT_PROMPT_AFTER_CAMPAIGN_027_SCAFFOLD.md`](NEXT_SPRINT_PROMPT_AFTER_CAMPAIGN_027_SCAFFOLD.md) as a separate, explicitly-instructed train/validation sprint.
- See [`CAMPAIGN_027_PRECOMMIT_H4_FILTERED_ZSCORE_REVERSION_SCOPE.md`](CAMPAIGN_027_PRECOMMIT_H4_FILTERED_ZSCORE_REVERSION_SCOPE.md), [`CAMPAIGN_027_H4_FILTERED_ZSCORE_REVERSION_SCAFFOLD_001_SUMMARY.md`](CAMPAIGN_027_H4_FILTERED_ZSCORE_REVERSION_SCAFFOLD_001_SUMMARY.md).

## 12a. CAMPAIGN_027 train/validation — REJECT_TRAIN_GATE; family CLOSED (2026-05-28)

- **Sprint.** `research-campaign-027-h4-filtered-zscore-reversion-train-validation-001` (train/validation evidence, **not** approval).
- **What.** Ran the frozen rule's train evidence (2020–2022) on the campaign's own ledgers under binding conservative cost; emitted the full edge-discovery-compatible artifact set (signal/trade/funnel/filter-stage ledgers, matched-null, filter-ablation, cost stress). No tuning, no matrix, no long side, no gate weakening.
- **Verdict.** **`REJECT_TRAIN_GATE`** — 4/8 binding train gates failed: profit factor 1.043 (<1.05); years non-negative 1/3 (only 2022); 2× cost-stress expectancy −0.00007745 (<0); filter-ablation `f_strong_extension` only reduces sample (+0.000034, forking-path). Train expectancy +0.00011974 (wafer-thin), AUD_USD-dominated, USD_JPY negative. Matched-null **information** gate passed (above structure-matched null, pctl 90) = loses less than random, not profitable.
- **Validation NOT run** (confirmation, not a rescue); **test lockbox NOT opened** (2025–2026 sealed); Backtrader parity **DEFER_PARITY_REJECTED**.
- **Status.** `REJECT_TRAIN_GATE / TEST_LOCKBOX_CLOSED / NOT_APPROVED`. `approved_strategies.yaml` stays `approved: []`; paper/demo/live blocked; C011 null; C025/C026 REJECT.
- **Family decision.** The `h4_filtered_zscore_reversion` family — the **last surviving edge-discovery front-gate idea** — is **CLOSED**. Revival requires a **new external thesis or new data** (standing restart criteria), **not** a parameter tweak, a different filter set, a long side, or a re-mined window. The lab remains the mandated front gate for any future search.
- See [`CAMPAIGN_027_FINAL_INTERPRETATION.md`](CAMPAIGN_027_FINAL_INTERPRETATION.md), [`CAMPAIGN_027_TRAIN_RESULT.md`](CAMPAIGN_027_TRAIN_RESULT.md), [`CAMPAIGN_027_TRAIN_VALIDATION_001_SUMMARY.md`](CAMPAIGN_027_TRAIN_VALIDATION_001_SUMMARY.md).
